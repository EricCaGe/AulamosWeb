...
$sql = "
    SELECT
        ...
        COUNT(DISTINCT e.id_entrega) AS intentos_recibidos,
        SUM(CASE WHEN e.estado IN ('Calificada','calificada') THEN 1 ELSE 0 END) AS calificadas
    FROM actividades a
...
$sql .= "
    GROUP BY
        a.id_actividad, a.titulo, a.descripcion, a.configuracion_evaluacion,
        a.fecha_publicacion, a.fecha_limite, a.puntaje_maximo, a.estado,
...
$cerradas = count(array_filter($items, fn($x) => strtolower($x['estado']) === 'cerrada'));
$intentos = array_sum(array_map(fn($x) => (int)$x['intentos_recibidos'], $items));

$conexion->close();

function configEval($json) {
    if (!$json) return [];
...
<title>Mis Evaluaciones - Docente</title>
...
<div class="logo-section"><img src="../img/logo_g.png" alt="Logo Aulamos" class="logo-img"></div>
        ...
        <nav class="menu">
            ...
            <a href="mis_evaluaciones.php" class="menu-item active"><i class="fa-solid fa-file-circle-check"></i> Mis Evaluaciones</a>
...
<p>Consulta las evaluaciones creadas, respuestas e intentos de tus estudiantes.</p>
</div>
<div class="header-actions">
<a href="crear_evaluacion.php" class="btn-assistant" style="text-decoration:none;"><i class="fa-solid fa-plus"></i> Nueva evaluación</a>
...
<div class="summary-card"><div class="n"><?php echo $total; ?></div><div class="l">Total de evaluaciones</div></div>
<div class="summary-card"><div class="n"><?php echo $publicadas; ?></div><div class="l">Publicadas</div></div>
<div class="summary-card"><div class="n"><?php echo $cerradas; ?></div><div class="l">Cerradas</div></div>
<div class="summary-card"><div class="n"><?php echo $intentos; ?></div><div class="l">Intentos recibidos</div></div>
</div>

<form class="page-tools" method="GET">
<div class="search-box"><i class="fa-solid fa-search"></i><input type="text" name="buscar" value="<?php echo htmlspecialchars($buscar); ?>" placeholder="Buscar evaluación, materia o curso..."></div>
<div class="filters">
...
<div class="cards">
<?php if (empty($items)): ?>
<div class="empty"><i class="fa-regular fa-file-lines"></i><h3>No hay evaluaciones</h3><p>No se encontraron evaluaciones con los filtros seleccionados.</p><a class="btn-mini primary" href="crear_evaluacion.php">Crear evaluación</a></div>
<?php else: foreach ($items as $item): $cfg = configEval($item['configuracion_evaluacion']); ?>
<div class="item-card">
<div class="item-top">
...
<div class="info"><strong>Intentos recibidos</strong><?php echo (int)$item['intentos_recibidos']; ?></div>
<?php if (isset($cfg['duracion_minutos'])): ?><div class="info"><strong>Duración</strong><?php echo (int)$cfg['duracion_minutos']; ?> min</div><?php endif; ?>
<?php if (isset($cfg['intentos_permitidos'])): ?><div class="info"><strong>Intentos permitidos</strong><?php echo (int)$cfg['intentos_permitidos']; ?></div><?php endif; ?>
</div>
<div class="actions">
...
<a class="btn-mini" href="calificar_entrega.php?id_actividad=<?php echo (int)$item['id_actividad']; ?>"><i class="fa-solid fa-graduation-cap"></i> Revisar respuestas</a>
</div>
</div>
